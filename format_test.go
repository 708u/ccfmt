...
func TestSettingsJSONFormatterDoesNotAddProjects(t *testing.T) {
	t.Parallel()
	input := `{"key": "value"}`
	result, err := NewSettingsJSONFormatter(NewPermissionSweeper(testutil.AlwaysTrue{}, "")).Format(t.Context(), []byte(input))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	got := string(result.Data)
...
func TestFormatRespectsContextCancellation(t *testing.T) {
	...
	f := NewClaudeJSONFormatter(testutil.AlwaysTrue{})
